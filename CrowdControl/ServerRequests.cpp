...
pplx::task<void> ServerRequests::SendPut(const std::wstring& putType, std::function<void(const std::wstring&)> callback, web::json::value json, bool gameSession) {
	std::wstring url;

	if (gameSession) {
		url = L"https://openapi.crowdcontrol.live/game-session/" + putType;
	}
	else {
		url = L"https://openapi.crowdcontrol.live/" + putType;
	}
